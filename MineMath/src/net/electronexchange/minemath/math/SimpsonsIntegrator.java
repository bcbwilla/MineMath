...
public class SimpsonsIntegrator implements Integrator
{
    ...
    public double integrate(double a, double b, int intervals)
    {
        ...
        double h         = (b - a)/intervals/2;  // interval width
                                                //   (split in two)
        double totalArea = 0;
    ...
    {
        double x2   = x1 + h;               // middle
        double x3   = x2 + h;               // right bound of the region
        double y1   = integrand.at(x1);     // value at left bound
        double y2   = integrand.at(x2);     // value at the middle
        double y3   = integrand.at(x3);     // value at right bound
        double area = h*(y1 + 4*y2+ y3)/3;  // area of the region
